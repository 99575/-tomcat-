...
public class ParseUrlPattern {
	...
	private static Map<String, String> urlPattern = new HashMap<String, String>();// 解稀后的映射
	...
	private void parseXml(String projectName, File childFile) {
		...
		try {
			...
			for (Element el : mimes) {
				urlPattern.put("/" + projectName + el.selectSingleNode("url-pattern").getText().trim(),
						el.selectSingleNode("servlet-class").getText().trim());
			}
